...
def report_valtest_adversarial_text(
    gini_value: float,
    gini_std: float,
    gini_spread: tp.Tuple[float, float],
    ...
    Args:
        ...
        gini_spread: P05/P95 значений по повторным разбиениям
    ...
    result_dataframe = pd.DataFrame(
        {
            ...
            "P05 Gini по разбиениям": [round(float(gini_spread[0]), 4)] if not np.isnan(gini_spread[0]) else [None],
            "P95 Gini по разбиениям": [round(float(gini_spread[1]), 4)] if not np.isnan(gini_spread[1]) else [None],
            "Светофор": [color],
        }
    )
    return {
...
def _not_computable(
    ...
    *,
    ...
    precomputed = {
        ...
        "gini_value": np.nan,
        "gini_std": np.nan,
        "gini_spread_lower": np.nan,
        "gini_spread_upper": np.nan,
...
def valtest_adversarial_text(
    ...
    **kwargs,
    ...
    5. Считается среднее, стандартное отклонение и P05/P95 по разбиениям
    """
    ...
    if gini_value is None:
        ...
        gini_spread = (
            float(np.percentile(gini_array, 5)),
            float(np.percentile(gini_array, 95)),
        )
    else:
    ...
    precomputed = {
        ...
        "gini_std": gini_std,
        "gini_spread_lower": gini_spread[0],
        "gini_spread_upper": gini_spread[1],
    ...
    report = report_valtest_adversarial_text(
        gini_value, gini_std, gini_spread, semaphore_threshold, is_info
    )
