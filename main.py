...
def html_report_valtest_oos_oot(res: dict, semaphore_title: str) -> str:
    ...
    return render_test_report(
        "6.3.7", "Разделение выборок OOS–OOT посредством стандартной модели",
        ...
        ("СЗ выше E; не менее 100 независимых единиц в каждой выборке.",
         "Диалоги и сессии не пересекаются между обучающей и тестовой частями.",
         "Оценки Автоасессора в признаки модели не входят."),
        [("green", "Gini < 0,4"), ("yellow", "0,4 ≤ Gini < 0,8"), ("red", "Gini ≥ 0,8"),
         ("gray", "Недостаточно независимых единиц, оценка недоступна, ширина доверительного "
          "интервала больше 0,2 или выбран информационный режим")],
        algorithm=(
            "Присвоить запросам OOS метку 0, запросам OOT — метку 1.",
            "Разделить данные на обучение и тест по группам диалогов без пересечения.",
            "Обучить CatBoost по текстам запросов и вычислить Gini = max(0, 2 × AUC − 1).",
            "Повторить расчёт на нескольких разбиениях и получить среднее, стандартное отклонение и доверительный интервал Gini.",
        ),
